...
typedef int atomic_int; //TODO: comment this!!!
...
int atomic_fetch_add(atomic_int *x, int arg)
{
    ...
    while (needReWrite) {
        oldValue = load_linked(x);
        needReWrite = !store_conditional(x, oldValue + arg);
    }
                             ...
                             int new_value)
{
    ...
    while (needReWrite) {
        oldValue = load_linked(x);
        if (*expected_value == oldValue) {
            needReWrite = !store_conditional(x, new_value);
            result = true;
        } else {
    ...
    return result;    // atomic_compare_exchange(2628, 2586, 2628) must not return true
}
